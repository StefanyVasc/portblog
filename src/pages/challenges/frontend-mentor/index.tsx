...
import { CustomBreadcrumb, Header, Tabs, TabsContent, TabsList, TabsTrigger } from "@/components";
...
export function FrontendMentor() {
  ...
  return (
    <div>
      ...
      <main>
        <Tabs value={currentTab} onValueChange={handleTabChange} className="w-[400px] mt-7">
          <TabsList>
            {tabsInfos.map((tab) => (
          ...
          <TabsContent key={currentTab} value={currentTab} className="mt-5 ml-3">
            <Outlet />
          </TabsContent>
        </Tabs>
      </main>
